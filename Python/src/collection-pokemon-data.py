...
def sv_pokemon_list(url: str):
    ...
    res_race = requests.get(url, timeout=1, headers=headers)
    # エラーチェック
...
def get_pokemon_status(soup_race, pokemon_id :int, pokemon_name :str):
...
def get_waza_list(soup_race, pokemon_id :int , pokemon_name :str):
...
def get_pokemon_info(pokemon_url :str):
    pokemon_id = int(pokemon_url.replace("https://yakkun.com/sv/zukan/n", ""))
    headers = {"User-Agent": "hoge"}
    res_race = requests.get(pokemon_url, timeout=1, headers=headers)
    # エラーチェック
    res_race.raise_for_status()
    soup_race = bs4.BeautifulSoup(res_race.content, "lxml")
...
if __name__ == "__main__":
    ...
    for pokemon_url in pokemon_list[:2]:
